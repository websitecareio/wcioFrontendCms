 ...
 * Smarty plugin
 * -------------------------------------------------------------
 * File:     modifier.genthumbnail.php
 * Type:     modifier
 * Name:     genthumbnail
 * Purpose:  Generates thumnails based on the size displayed on site.
 * -------------------------------------------------------------
 */
...
function smarty_modifier_genthumbnail($src,$width,$height,$name)
{
// No need to check if the image is an image here, it will just slow down the size. On the user upload process it needs to be checked tho.

if(!file_exists(dirname(__FILE__)."/../../$src") || $src == '' || $image == "0") {
  return $src."?file-not-found";
}

$strReplaceName = str_replace(' ', '-', $name); // Replaces all spaces with hyphens.
$subNewFileName = "".$strReplaceName."-".$width."-".$height."";
$newFilename = strtolower(preg_replace('/[^A-Za-z0-9\-]/', '', $subNewFileName)).".png"; // Removes special chars.

if(!file_exists(dirname(__FILE__)."/../../uploads/$newFilename")) {


  // Open JPG image
  $magicianObj = new imageLib(dirname(__FILE__)."/../../$src");

  // Resize to best fit then crop
  $magicianObj->resizeImage($height, $width, "crop");

  // Save resized image as a PNG
  $magicianObj->saveImage(dirname(__FILE__)."/../../uploads/$newFilename");
